...
# Set your Cosm feed id and key here..
my $feed_id = '*****';
my $feed_key = '**************************';
...
# my $arb  = $serialport->can_arbitrary_baud;
...
# set up feed..
my $pachube = Net::Pachube->new( key => $feed_key, );
my $feed = $pachube->feed($feed_id);

print "Writing to feed id $feed_id - ", $feed->title, ", Status: ", $feed->status unless $quiet;
...
while (1) {
    ...
    # power is in kW:
    $avg_power = ($avg_amps * $voltage)/1000;
    # format to 2 decimals..
    $avg_power = sprintf("%.2f", $avg_power);
    
    $feed->update( data => [$avg_amps, $avg_power] )
    || print 'Cosm update failed!';
